...
from PIL import Image, ImageTk  # Pillow é a biblioteca atualizada para manipulação de imagens
import cv2
import usb.core
import usb.util
...
def mostrar_video():
    if cap.isOpened():
        ...
        if ret:
            ...
            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  # Converte BGR (OpenCV) para RGB (Tkinter)
            img = Image.fromarray(frame)  # Converte o frame para uma imagem do Pillow
            img_tk = ImageTk.PhotoImage(image=img)  # Converte para um objeto Tkinter
        ...
        # Chama a função novamente após 10ms para atualizar o vídeo
...
def sair():
    global cap
    if cap is not None and cap.isOpened():  # Verifica se cap foi inicializado e se está aberto
        cap.release()  # Liberta a câmara se estiver ligada
